...
main_dir = 'C:\Users\sacha.bucourt\Documents\Data lab\CHIRP\260522\16-01-52_calcite_2ps'

sub_dir  = '';
sub_dir_save = [main_dir '/extracted_data/' sub_dir];

title_fig = 'calcite geologique no chirp';

wavelengths =[927.2,927.7,928.1,928.6,928.9,929.5,929.8,930.4,926.8,926.3,925.9,925.5];
...
%% Save spectrum vectors
cd(main_dir);
if exist(sub_dir_save, 'dir') == 0
    mkdir(sub_dir_save)
end
save([sub_dir_save '/spectrum_' title_fig '.mat'], 'wavenumber_sorted', 'sp_sorted');
disp(['Spectre sauvegardé : ' sub_dir_save '/spectrum_' title_fig '.mat']);
...
superpose_spectre_Calcite(wavenumber_sorted,sp_sorted,7.4)
...
wn_min_fit = 1060;
wn_max_fit = 1100;
mask_roi = wn >= wn_min_fit & wn <= wn_max_fit;
wn = wn(mask_roi);
sp = sp(mask_roi);
...
%% Overlay two fits
% fit2 = load('C:\Users\sacha.bucourt\Documents\Data lab\CHIRP\260529\17-15-15_Cal_7ps\extracted_data\fit_calcite geologique - chirp 7ps.mat');  % <-- adjust path
% f2   = fit2.fit_results;
% 
% figure('Color','white','Position',[100 100 900 580]);
% 
% sp1_norm = (fit_results.sp - min(fit_results.sp)) / (max(fit_results.sp) - min(fit_results.sp));
% plot(fit_results.wn, sp1_norm, 'r.', 'MarkerSize', 8); hold on;
% 
% sp2_norm = (f2.sp - min(f2.sp)) / (max(f2.sp) - min(f2.sp));
% plot(f2.wn, sp2_norm, 'b.', 'MarkerSize', 8);
% 
% colors1 = lines(fit_results.n_peaks);
% colors2 = cool(f2.n_peaks);
% 
% gauss_offset = @(a,b,c,d,x) a*exp(-((x-b)/c).^2) + d;
% 
% for k = 1:fit_results.n_peaks
%     if isnan(fit_results.lambda0(k)), continue; end
%     a = fit_results.amplitude(k);
%     b = fit_results.lambda0(k);
%     c = fit_results.FWHM_fit(k) / (2*sqrt(log(2)));
%     d = fit_results.offset(k);
%     sp1_range = max(fit_results.sp) - min(fit_results.sp);
%     xx = linspace(b - 3*c, b + 3*c, 300);
%     yy = (gauss_offset(a,b,c,d,xx) - min(fit_results.sp)) / sp1_range;
%     plot(xx, yy, '-', 'Color', colors1(k,:), 'LineWidth', 2.0);
%     text(b, max(yy)+0.04, sprintf('%.1f cm^{-1}\nFWHM=%.2f', b, fit_results.FWHM_fit(k)), ...
%         'Color', colors1(k,:), 'FontSize', 9, 'HorizontalAlignment', 'center');
% end
% 
% for k = 1:f2.n_peaks
%     if isnan(f2.lambda0(k)), continue; end
%     a = f2.amplitude(k);
%     b = f2.lambda0(k);
%     c = f2.FWHM_fit(k) / (2*sqrt(log(2)));
%     d = f2.offset(k);
%     sp2_range = max(f2.sp) - min(f2.sp);
%     xx = linspace(b - 3*c, b + 3*c, 300);
%     yy = (gauss_offset(a,b,c,d,xx) - min(f2.sp)) / sp2_range;
%     plot(xx, yy, '--', 'Color', colors2(k,:), 'LineWidth', 2.0);
%     text(b, max(yy)+0.08, sprintf('%.1f cm^{-1}\nFWHM=%.2f', b, f2.FWHM_fit(k)), ...
%         'Color', colors2(k,:), 'FontSize', 9, 'HorizontalAlignment', 'center');
% end
% 
% xlabel('Wavenumber (cm^{-1})', 'FontSize', 13);
% ylabel('Raman Intensity (normalized)', 'FontSize', 13);
% title('Fit comparison', 'FontSize', 14);
% legend({fit_results.title, f2.title}, 'Location', 'northwest', 'FontSize', 11);
% xlim([min([fit_results.wn; f2.wn])-5, max([fit_results.wn; f2.wn])+5]);
% ylim([0 1.25]);
% grid on; box on;
% set(gca,'FontSize',11);
