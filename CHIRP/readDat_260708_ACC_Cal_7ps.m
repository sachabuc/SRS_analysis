...
if ROI_known == true 
    ...
    x_bg=73; y_bg=91; width_bg=6; height_bg=7;
    x_sig1=61; y_sig1=22; width_sig1=8; height_sig1=9;
    x_sig2=97; y_sig2=78; width_sig2=8; height_sig2=9;
...
tau_fwhm = 4.27;
sub_dir_save='C:\Users\sacha\OneDrive\Documents\FRESNEL\CHIRP\260708\14-55-09_ACC_Cal - Copie\extracted_data';
...
R2_min = 0.65;
residual_sigma_max = 5;


% signal_conv = models_CC(...
%     wavenumber_sorted,...
%     sp_sum_sorted,...
%     tau_fwhm,...
%     true,...
...
[phase_map, A_maps, ratio_maps] = ...
    ...
    min_points_bckg,...    %min_points_bckg
    threshold_ratio, ...              % seuil ACC/Calcite
    true,...
    sub_dir_save,...
    R2_min, ...
    residual_sigma_max);                 % affichage

[phase_map_7ps_shifted, ...
    ...
    sub_dir_save, ...          % dossier contenant phase_map_2ps.mat
    shift_x, ...
    shift_y, ...
    I_corr, ...
    wavenumber, ...
...
title_fig = 'fit fwhm_inst';

% --- Clean data ---
valid = isfinite(sp_sum_sorted) & isfinite(wavenumber_sorted(:));
wn = wavenumber_sorted(valid);
sp = sp_sum_sorted(valid);
...
for k = 1:n_peaks
    ...
    annotation('textbox', [0.70 ypos 0.25 0.05], ...
        'String', txt, ...
        'FitBoxToText','on', ...
        'BackgroundColor','w', ...
        'EdgeColor','k', ...
