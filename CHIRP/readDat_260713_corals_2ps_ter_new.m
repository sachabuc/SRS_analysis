...
main_dir = 'C:\Users\sacha.bucourt\Documents\Data lab\CHIRP\260713\13-49-00_coral_2ps_ter'
% main_dir = 'C:\Users\sacha\OneDrive\Documents\FRESNEL\CHIRP\260708\14-55-09_ACC_Cal - copie';
...
wavelengths =[928.3,928.7,929.1,929.5,929.9,930.3,931,928.1,927.8,927.5,...
    927.2,926.7,926.3,925.8];  % nm, ordre d'acquisition (pas trie)
...
fwhm_mesure = 14.4;
fwhm_calcite = 3.5;
[f_calc, tau_fwhm] = deconvolve_voigt(fwhm_mesure, fwhm_calcite, 0);
tau_fwhm  = 10;   % ps
...
nu_LB   = [1085, 1084, 1073, 1073,  1097, 1066];   % cm^-1
nu_UB   = [1086.5, 1086.5, 1093, 1082,  1102, 1070];   % cm^-1
FWHM_LB = [tau_fwhm    ,tau_fwhm    , tau_fwhm    , 20, tau_fwhm    , tau_fwhm     ];   % cm^-1
FWHM_UB = [tau_fwhm + 4,tau_fwhm + 4, tau_fwhm + 6, 40, tau_fwhm + 4, tau_fwhm + 4 ];   % cm^-1
...
R2_min_final            = 0.7;   % R^2 minimal pour garder un pixel (filtre reel, ici)
threshold_sigma_final   = 3;     % facteur x noise_map pour compter un point "au-dessus du bruit"
min_points_above_noise  = 3;     % nb minimal de points au-dessus du bruit pour garder le pixel
...
calcul_tau_instr = true; 
do_noise_study = true;
do_lsqnonneg_treatment = true; 
...
%% ---- 2.1 Chargement ----------------------------------------------------
[imgs, acq] = loadSRSImageStack(main_dir, sub_dir, n_depart);
...
pixel_fit = fit_pixel_phases( ...
    ...
    nu_is_variable, FWHM_is_variable, tau_fwhm, ...
...
ref_spectra = plot_phase_reference_spectra( ...
          ...
          phase2plot, nbr_pix_per_phase,tau_fwhm,false);
...
for i = 1:numel(ref_spectra)
    ...
    for k = 1:size(rs.A_values, 2)
        ...
        fprintf('  Phase %-3d : A_mean = %.5g   A_std = %.5g\n', ...
            k, rs.A_mean(k), rs.A_std(k));

    end


    % Parametres de la phase cible
    ...
    if ~isempty(rs.FWHM_target_mean)
        ...
        for j = 1:numel(rs.FWHM_target_mean)
            ...
            fprintf('  Raie %d : FWHM = %.4f +/- %.4f cm^-1\n', ...
                j, ...
                rs.FWHM_target_mean(j), ...
                rs.FWHM_target_std(j));
