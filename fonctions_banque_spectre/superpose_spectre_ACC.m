function superpose_spectre_ACC(wavenumber_sorted, sp_sorted, tau_fwhm)
%% superpose_spectre_ACC
...
%% --- Modélisation par somme de Gaussiennes ---
% gaussienne = @(A, x0, sigma, x) A .* exp(-((x - x0).^2) / (2 * sigma^2));
% pic1     = gaussienne(1, 1086.0, 2, wavenumber);   % sigma = FWHM / 2√(2ln2) ≈ FWHM / 2.355
...
pic1     = lorentzienne(1, 1078, 15, wavenumber);
...
if tau_fwhm > 0
    title(sprintf('Spectre SRS – ACC  |  instrumental response FWHM = %.1f cm^{-1}', tau_fwhm), ...
          'FontSize', 13);
else
    title('Spectre Raman – ACC', 'FontSize', 14);
end
...
% Légende adaptative
if tau_fwhm > 0
    legend({'theoritical ACC', ...
            sprintf('theoritical ACC \\otimes instrumental response'), ...
            'Experimental measurements'}, ...
           'Location', 'northwest', 'FontSize', 11);
else
    legend({'ACC', 'Expérimental'}, ...
           'Location', 'northwest', 'FontSize', 12);
end
 
% disp(length(wn_kernel))  % doit être impair
% disp(wn_kernel(ceil(end/2)))  % doit être 0 (ou très proche)
