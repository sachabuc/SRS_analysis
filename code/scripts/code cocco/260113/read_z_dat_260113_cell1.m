...
main_dir = 'C:\Users\sacha.bucourt\Documents\Data lab\spectres\campagne cocco\260113\cell1'
sub_dir  = '';
...
wavelengths = [927,928.6,925.8,929.2,927.3,928.6]; 
% 
...
% [927,928.6,925.8,929.2,927.3,928.6]
    ...
    I2 = squeeze(I_corr(n2,:,:)); % fluo 
    I3 = squeeze(I_corr(n3,:,:)); % ACChira inf
    ...
    subplot(3,3,5);
    imagesc(I3-I1); caxis([-0.5 1]); colorbar;
    title('(I3-I1)');
    hold on;
    contour(I1, 1, 'w', 'LineWidth', 1);
    hold off;
    ...
    subplot(3,3,6);
    imagesc(I3-I2); caxis([-0.5 1]); colorbar;
    title('I3-I2');
    hold on;
    contour(I1, 1, 'w', 'LineWidth', 1);
    hold off;
    ...
    subplot(3,3,7);
    imagesc(I2-I1); caxis([-1 3]); colorbar;
    title('I2-I1');
    hold on;
    contour(I1, 1, 'w', 'LineWidth', 1);
    hold off;
    ...
    subplot(3,3,8);
    imagesc(I3-I4); caxis([-1 1]); colorbar;
    title('I3-I4');
    hold on;
    contour(I1, 1, 'w', 'LineWidth', 1);
    hold off;
    ...
    subplot(3,3,9);
    imagesc(I4-I1); caxis([-0.5 1]); colorbar;
    title('I4-I1');
    hold on;
    contour(I1, 1, 'w', 'LineWidth', 1);
    hold off;
    ...
    % choisir stack
    image_Zi = imgs_Zpermute(14,:, :, :); % Si imgs_Z est déjà de taille (Nx, Ny, Nlambda), pas besoin de changer
    ...
    % 
    % [A_exp,tau_exp,Imean,Cst] = extract_fluo_cst(image_Zi, n2);
    % [A_exp2,tau_exp2,Imean2] = extract_fluo_cst(image_Zi, n2);
    % 
    % correctfluo_intensityprofile(image_Zi,wavenumber,n3,A_exp,tau_exp,Imean,0)
    % correctfluo_intensityprofile(image_Zi,wavenumber,n3,A_exp2,tau_exp2,Imean2,0)

    % intensityprofile(image_Zi,wavenumber,2);

    % intensityprofile(image_Zi,wavenumber,n3)

end

%% Correction Fluo 
    [A_exp,tau_exp,Imean,Cst] = extract_fluo_cst(image_Zi, n2, 0, 20);
    % [A_exp2,tau_exp2,Imean2,Cst2] = extract_fluo_cst(imgs, n2);
    % [A_exp3,tau_exp3,Imean3,Cst3] = extract_fluo_cst(imgs, n2);

    % [A_expbis,tau_expbis,Imeanbis] = extract_fluo(imgs, n2);
    % [A_expbis2,tau_expbis2,Imeanbis2] = extract_fluo(imgs, n2);
    % [A_expbis3,tau_expbis3,Imeanbis3] = extract_fluo(imgs, n2);


    imgs_fluo_corr = correctfluo_intensityprofile_virginie2(image_Zi,n2,A_exp,tau_exp,Cst);
    % correctfluo_intensityprofile(imgs,wavenumber,n3,A_expbis,tau_expbis,Imeanbis,0)



    I1fc = imgs_fluo_corr(:,:,n1); % calcite
    I2fc = imgs_fluo_corr(:,:,n2); % fluo (CCHH)
    I3fc = imgs_fluo_corr(:,:,n3); % ACChira inf
    I4fc = imgs_fluo_corr(:,:,n4); % inf ACC 

    figure(103);
    
    subplot(2,2,1);
    imagesc(I1fc); caxis([-2 3]); colorbar;
    title('calcite (I1)');
    hold on;
    contour(I1, 1, 'w', 'LineWidth', 1);
    hold off;
    ...
    subplot(2,2,2);
    imagesc(I3fc); caxis([-2 3]); colorbar;
    title('ACChira inf (I3)');
    hold on;
    contour(I1, 1, 'w', 'LineWidth', 1);
    hold off;

    subplot(2,2,3);
    imagesc(I2fc); caxis([-2 3]); colorbar;
    title('fluo (I2)');
    hold on;
    contour(I1, 1, 'w', 'LineWidth', 1);
    hold off;
    ...
    subplot(2,2,4);
    imagesc(I3fc-I1fc); caxis([-2 3]); colorbar;
    title('I3-I1');
    hold on;
    contour(I1, 1, 'w', 'LineWidth', 1);
    hold off;

    sgtitle('Images SRS (fluo corrigé) associées aux wavenumbers de : Calcite, ACC-H₂O et Fluorescence', 'FontSize', 12, 'FontWeight', 'bold');
    
    intensityprofile(imgs_fluo_corr,wavenumber,n3,10,20,0,0,0,0);
...
z_list = (7:17);
% Définir une liste de marqueurs pour différencier les courbes
markers = {'o', 's', 'd', '^', 'v', '<', '>', 'p', '*', '+', 'x'};

% Créer un dégradé de gris pour les couleurs
n_shades = length(z_list);
gray_shades = linspace(0.1, 0.9, n_shades); % De gris foncé à gris clair
...
for iz = 1:length(z_list)
    ...
    % Tracé avec un marqueur distinct pour chaque point de la courbe
    marker_idx = mod(iz-1, length(markers)) + 1;
    plot(wavenumber_sorted, sp_sorted, ...
         'LineWidth', 1.2, ...
         'Marker', markers{marker_idx}, ...
         'MarkerSize', 5, ...
         'MarkerEdgeColor', [gray_shades(iz) gray_shades(iz) gray_shades(iz)], ...
         'MarkerFaceColor', [gray_shades(iz) gray_shades(iz) gray_shades(iz)], ...
         'Color', [gray_shades(iz) gray_shades(iz) gray_shades(iz)]);
    % 'MarkerIndices' est retiré pour afficher un marqueur à chaque point
...
title('Spectrum for selected Z planes', 'FontSize', 14, 'FontWeight', 'bold');
grid on;
set(gca, 'FontSize', 10); % Taille de police pour les axes
