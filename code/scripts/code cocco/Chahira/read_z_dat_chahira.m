...
main_dir = 'C:\Users\sacha.bucourt\Documents\Data lab\mesure cocco Chahira\250701\maybe no signal (gocea)';
sub_dir  = '';
...
wavelengths = [925.6, 926, 926.7, 927, 927.3, 927.5, 928, 928.5, 929, 929.5, 930, 930.4, 931, 931.4, 932, 932.4, 932.9, 933.3, 933.9, 934.2, 934.9, 935.3, 935.8, 936.5, 936.8, 937.7, 938.2, 938.4, 938.8, 939, 939.5, 940, 940.6, 941.1,941.1, 941.6, 941.9, 942.3, 943.1];

wavelengths = [933.9,934.2,934.9,935.3,935.8,936.5,936.8,937.3,937.7,938.2,938.8,938.4,939,939.5,940,940.6,941.1,941.6,941.9,942.3,927,943.1,928,927.5,928.5,929,929.5,930,930.4,931,931.4,932,932.4,932.9,933.3,933.8,927.3,926.7,926,925.6];
...
%Contrast
    ...
    I3 = squeeze(I_corr(n3,:,:)); % ACC - H2O
    I4 = squeeze(I_corr(n4,:,:)); % ACChira inf
    ...
    subplot(3,3,6);
    imagesc(I5-I1); caxis([-0.5 1]); colorbar;
    title('I5-I1');

    subplot(3,3,7);
    imagesc(I5); caxis([-1 3]); colorbar;
    title('inf ACC (I5)');

    subplot(3,3,8);
    imagesc(I2-I1); caxis([-1 1]); colorbar;
    title('I2-I1');

    subplot(3,3,9);
    imagesc(I3-I5); caxis([-0.5 1]); colorbar;
    title('I3-I5');

    imgs_Zpermute = permute(imgs_Z, [4,2, 3, 1]); % Réorganiser en (nZ,Nx, Ny, Nlambda) pour adapter aux fonctions suivantes
    
    % choisir stack
    image_Zi = imgs_Zpermute(4,:, :, :); % Si imgs_Z est déjà de taille (Nx, Ny, Nlambda), pas besoin de changer
    ...
    intensityprofile(image_Zi,[],10,10,10,0);

    % [A_exp,tau_exp] = extract_fluo(image_Zi, n2);
    % 
    % correctfluo_intensityprofile(image_Zi,wavenumber,n3,A_exp,tau_exp)
