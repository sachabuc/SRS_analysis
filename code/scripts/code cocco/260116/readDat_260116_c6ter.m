...
main_dir = 'C:\Users\sacha.bucourt\Documents\Data lab\spectres\campagne cocco\260119\09-55-03_cell6bis';
...
wavelengths=[927, 930.3, 928.1, 928.5]; 
...
if contrast == true
    ...
    % I5 = I_corr(:,:,n5); % polyphosphate 2 sup 
    C21=(I2-I1)./(I1+I2);
    C23=(I2-I3)./(I2+I3);
    C24=(I2-I4)./(I2+I4);
    % C4=(I4-I2);
    ...
    % Contraste binné
    C23_bin = (I2_bin - I3_bin) ./ (I2_bin + I3_bin);
    % C45_bin = (I4_bin - I5_bin) ./ (I4_bin + I5_bin);
    
    %% Correction shift 

    % shift déterminer à l'oeil 
    I1sc = I1; % calcite
    I2sc = correct_xy_shift(I2,0,0); % fluo 
    I3sc = correct_xy_shift(I3,0,0); % ACC H2O 
    I4sc = correct_xy_shift(I4,0,0); % ACC Chira
    % I5sc = correct_xy_shift(I5,0,0); % inf fluo 2
    ...
    maps_dir = fullfile(main_dir, 'fluo_maps_no_cal_no_shift');  % Dossier dédié aux maps
    ...
    mask_good_fit = analyze_map_fluo(A_map, tau_map, C_map, Imean_map, R2_map,0.99);
    ...
    % subplot(2,3,5);
    % imagesc(I5); caxis([-5 Imax]); colorbar;
    % title('ACC inf (I5)');
    % hold on;
    % contour(I1, 3, 'w', 'LineWidth', 1);
    %     % Superposition des pixels avec R² > seuil_R2
    ...
    sgtitle('Images SRS associées aux wavenumbers de : Calcite, ACC-H₂O et Fluorescence', 'FontSize', 12, 'FontWeight', 'bold');
    ...
    subplot(2,3,1);
    imagesc(I1sc); caxis([Imin Imax]); colorbar;
    title('calcite (I1)');
    hold on;
    ...
    A_seuil = 1000; % filtre les valeurs aberrantes 
    tau_seuil = 10;
    R2_seuil=0.99; % filtre les fits de mauvaise qualité 

    count = 0; % nombre de fit sélectionné 
    ttau_best = zeros(x2_ROI-x1_ROI+1,y2_ROI-y1_ROI+1);
    tA_best = zeros(x2_ROI-x1_ROI+1,y2_ROI-y1_ROI+1);
    tC_best = zeros(x2_ROI-x1_ROI+1,y2_ROI-y1_ROI+1); 
    ...
    imgs_fluo_corrA = correctfluo_intensityprofile_virginieA(imgsc,n2,A_best,tau_best,C_best);
...
%% Affichage images SRS fluo corrigé 
    ...
    subplot(2,3,1);
    imagesc(I1fcA); caxis([Imin Imax]); colorbar;
    title('calcite (I1)');
    hold on;
    contour(I1, 3, 'w', 'LineWidth', 1);
    hold off;
    ...
    subplot(2,3,6);
    imagesc(I3fcA-I1fcA); caxis([-5 20]); colorbar;
    title('I3-I1');
    hold on;
    contour(I1, 3, 'w', 'LineWidth', 1);
    hold off;
    ...
    % [ROI_sig_coord_corr,corr_Iprofile] = intensityprofile(imgs_fluo_corr,wavenumber,3,10,10,139,64,3,3); % ROI ACC
    [ROI_sig_coord_corrA,corr_IprofileA] = intensityprofile(imgs_fluo_corrA,wavenumber,3,10,10,139,64,3,3); % ROI ACC
    [ROI_sig_coord,Iprofile] = intensityprofile(imgs,wavenumber,3,10,10,139,64,3,3); % n_ACC,x_ROI_bckg,y_roi_bckg,x_ROI_signal,y_ROI_signal,w_ROI_signal,h_ROI_signal
    ...
    legend('correction modèle avec A', 'Profil original','original - sans A', 'Location', 'best');
