...
main_dir = 'C:\Users\sacha.bucourt\Documents\Data lab\spectres\campagne cocco\260116\16-37-36_cell6';
...
%260114a Z0
wavelengths=[927.0, 930.5, 928,928.6,930.4]; 
...
if contrast == true
   %% Correction bckg 
    ...
    % Fonction anonyme de binning 2x2
    ...
    %% Décomposition en composante principal (ne marche pas bien)
    ...
    % shift déterminer à l'oeil  (-1,-3)
    % I1sc = I1; % calcite
    % I2sc = correct_xy_shift(I2,-1,-3); % fluo 
    % I3sc = correct_xy_shift(I3,-1,-3); % ACC H2O 
    % I4sc = correct_xy_shift(I4,-1,-3); % ACC Chira
    % I5sc = correct_xy_shift(I5,-1,-3); % fluo 2

    I1sc = I1; % calcite
    I2sc = correct_xy_shift(I2,0,0); % fluo 
    I3sc = correct_xy_shift(I3,0,0); % ACC H2O 
    I4sc = correct_xy_shift(I4,0,0); % ACC Chira
    I5sc = correct_xy_shift(I5,0,0); % fluo 2
    ...
    maps_dir = fullfile(main_dir, 'fluo_maps');  % Dossier dédié aux maps
    ...
    if exist(filename_A, 'file') && exist(filename_tau, 'file') && ...
       exist(filename_C, 'file') && exist(filename_Imean, 'file') && exist(filename_R2, 'file')
        ...
        [A_map, tau_map, C_map, Imean_map, R2_map] = extract_map_fluo(imgsc_no_cal);
    ...
    subplot(2,3,1);
    imagesc(I1); caxis([Imin 70]); colorbar;
    title('calcite (I1)');
    hold on;
    ...
    subplot(2,3,6);
    imagesc(I3-I1); caxis([-5 5]); colorbar;
    title('I3-I1');
    hold on;
    contour(I1, 3, 'w', 'LineWidth', 1);
    hold off;    
    ...
    % Imin = -5;
    subplot(2,3,1);
    imagesc(I1sc); caxis([Imin 70]); colorbar;
    title('calcite (I1)');
    hold on;
    ...
    hold off;
        ...
        subplot(2,3,5);
    imagesc(I5sc); caxis([Imin Imax]); colorbar;
    title('fluo 2 (I5)');
    hold on;
    contour(I1, 3, 'w', 'LineWidth', 1);
        % Superposition des pixels avec R² > seuil_R2
    ...
    A_seuil = 2000;
    tau_seuil = 2; % filtre les valeurs aberrantes 
    ...
    disp('Fit parameters extract from maps :')
    fprintf('  A_best = %.3g\n', A_best);
    fprintf('  tau_best = %.3g\n', tau_best);
    fprintf('  C_best = %.3g\n', C_best);
    fprintf('  N_sample = %.3g\n', count);
    ...
    [A_exp,tau_exp,Imean,Cst] = extract_fluo_cst(imgs, n2,5,5,70,80,30,30);
...
%% Etude de la correlation des paramètres du fit 
    ...
    % figure;
    % subplot(3,1,1);
    % A_hist = histogram(tA_best(:), 1000);
    % title('Histogramme de tA_best');
    % xlabel('Valeurs');
    % ylabel('Fréquence');
    % 
    % subplot(3,1,2);
    % tau_hist = histogram(ttau_best(:), 1000);
    % title('Histogramme de ttau_best');
    % xlabel('Valeurs');
    % ylabel('Fréquence');
    % 
    % subplot(3,1,3);
    % C_hist = histogram(tC_best(:), 1000);
    % title('Histogramme de tC_best');
    % xlabel('Valeurs');
    % ylabel('Fréquence');
...
%% Affichage images SRS fluo corrigé 
    ...
    % imgs_fluo_corrA = correctfluo_intensityprofile_virginieA(imgsc,n2,16.5,0.7,8.2);
    % imgs_fluo_corrA = correctfluo_intensityprofile_virginieA(imgs,n2,A_best,tau_best,C_best);

    imgs_fluo_corrA = correctfluo_julien(I_corr,p_fit(i,2));
    ...
    subplot(2,3,1);
    imagesc(I1fcA); caxis([Imin 70]); colorbar;
    title('calcite (I1)');
    hold on;
    contour(I1, 3, 'w', 'LineWidth', 1);
    hold off;
    ...
    subplot(2,3,3);
    imagesc(I3fcA); caxis([Imin Imax]); colorbar;
    title('ACC-H₂O (I3)');
    hold on;
    contour(I1, 3, 'w', 'LineWidth', 1);
    hold off;
    ...
    subplot(2,3,4);
    imagesc(I4fcA); caxis([Imin Imax]); colorbar;
    title('I4');
    hold on;
    contour(I1, 3, 'w', 'LineWidth', 1);
    hold off;
    ...
    subplot(2,3,5);
    imagesc(I5fcA); caxis([Imin Imax]); colorbar;
    title('fluo 2 (I5)');
    hold on;
    contour(I1, 3, 'w', 'LineWidth', 1);
    hold off;
    ...
    subplot(2,3,6);
    imagesc(I3fcA-I1fcA); caxis([-5 5]); colorbar;
    title('I3-I1');
    hold on;
    contour(I1, 3, 'w', 'LineWidth', 1);
    hold off;
...
%% Etude facteur d'atténuation 

attenuation_factor(imgsc,2);



    %% Etude profil d'intensité 
 
    % [ROI_sig_coord_corr,corr_Iprofile] = intensityprofile(imgs_fluo_corr,wavenumber,3,10,10,90,92,5,5); 
    % [ROI_sig_coord_corrA,corr_IprofileA] = intensityprofile(imgs_fluo_corrA,wavenumber,3,10,10,90,92,5,5);
    % [ROI_sig_coord,Iprofile] = intensityprofile(imgs,wavenumber,3,10,10,90,92,5,5); % n_ACC,x_ROI_bckg,y_roi_bckg,x_ROI_signal,y_ROI_signal,w_ROI_signal,h_ROI_signal
    % 
    % 
    % [ROI_sig_coord_corr,corr_Iprofile] = intensityprofile(imgs_fluo_corr,wavenumber,3,10,10,75,135,3,3); % ROI ACC
    % [ROI_sig_coord_corrA,corr_IprofileA] = intensityprofile(imgs_fluo_corrA,wavenumber,3,10,10,75,135,3,3); % ROI ACC
    % [ROI_sig_coord,Iprofile] = intensityprofile(imgs,wavenumber,3,10,10,75,135,3,3); % n_ACC,x_ROI_bckg,y_roi_bckg,x_ROI_signal,y_ROI_signal,w_ROI_signal,h_ROI_signal


    % [ROI_sig_coord_corr,corr_Iprofile] = intensityprofile(imgs_fluo_corr,wavenumber,3,10,10,124,113,5,5); %ROI newacc
    % [ROI_sig_coord_corrA,corr_IprofileA] = intensityprofile(imgs_fluo_corrA,wavenumber,3,10,10,124,113,5,5);
    % [ROI_sig_coord,Iprofile] = intensityprofile(imgs,wavenumber,3,10,10,124,113,5,5); % n_ACC,x_ROI_bckg,y_roi_bckg,x_ROI_signal,y_ROI_signal,w_ROI_signal,h_ROI_signal
    ...
    % grid on
    % 
    % legend('correction modèle sans A','correction modèle avec A', 'Profil original','original - sans A','original - sans A', 'Location', 'best');
