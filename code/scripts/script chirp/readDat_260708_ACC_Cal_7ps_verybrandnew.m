...
FWHM_UB = [     4,      4,   10,   40,   10,   10];   % cm^-1
 
lineshape_type  = 'gaussian';    % 'gaussian' pour l'instant ('lorentzian' a venir)
ci_alpha        = 0.1;          % niveau pour l'IC sur chaque amplitude (0.05 -> IC 95%)
...
path2imgs_2ps = 'C:\Users\sacha.bucourt\Documents\Data lab\CHIRP\260708\14-55-09_ACC_Cal - Copie\extracted_data\squeeze_imgs\squeeze_imgs';
imgs_2ps = load(path2imgs_2ps);
...
imgs_2ps = imgs_2ps.squeeze_imgs;

sumI_raw=sum(I_raw,3);
sum_imgs_2ps = sum(imgs_2ps,3); 

if automatic_shift

    mask_sumI_raw = create_rectangle_mask(size(sumI_raw), 40, 100,90, 125);
    mask_sum_imgs_2ps = create_rectangle_mask(size(sum_imgs_2ps), 40, 100,90, 125);

    [dx, dy, I_ref_crop, I_crop, I_diff, val_max] = find_shift_fft_norm( ...
          sum_imgs_2ps, sumI_raw,'highpass',mask_sum_imgs_2ps,mask_sumI_raw);
    shift_col = dx;
    shift_row = dy;

else 

    I_raw_shifted = shift_images (sumI_raw,sum_imgs_2ps,shift_col,shift_row);
...
%% ---- Shifted Spectra compare to 2ps  ----------------------------------
