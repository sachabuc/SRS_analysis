...
addpath('C:\Users\sacha\OneDrive\Documents\FRESNEL\MATLAB\MesFonctions')
...
% main_dir = 'C:\Users\sacha.bucourt\Documents\Data lab\CHIRP\260710\15-15-20_coral_2ps_ter'
main_dir = 'C:\Users\sacha\OneDrive\Documents\FRESNEL\CHIRP\260710\15-15-20_coral_2ps_ter'
...
wavelengths =[927.2,927.6,928,928.5,928.8,929.3,929.6,930,930.5,931,931.4,926.8,926.3,925.8];
...
[I_corr,SNR,sigma_spec] = remove_planar_background(imgs,10:20,10:20,true,1);
...
% fwhm_ACC = 20;
% 
% nu_ARA  = 1085;      % aragonite/calcite
% fwhm_ARA = 10;
% 
% % Conversion FWHM -> sigma
% sigma_ACC = fwhm_ACC / (2*sqrt(2*log(2)));
% sigma_ARA = fwhm_ARA / (2*sqrt(2*log(2)));
% 
% % =========================
% %  Fenêtre spectrale
% % ==========================
% 
% idx = (wavenumber >= 1050 & wavenumber <= 1105);
% 
% nu = wavenumber(idx);
% cube_sub = I_corr(:,:,idx);
% 
% [Ny, Nx, Nnu] = size(cube_sub);
% 
% % =========================
% %  Modèles gaussiens
% % ==========================
% 
% g_ACC = exp(-(nu - nu_ACC).^2/(2*sigma_ACC^2));
% g_ARA = exp(-(nu - nu_ARA).^2/(2*sigma_ARA^2));
% 
% % Normalisation
% g_ACC = g_ACC / norm(g_ACC);
% g_ARA = g_ARA / norm(g_ARA);
% 
% %% =========================
% %  Cartes de sortie
% % ==========================
% 
% score_ACC = zeros(Ny,Nx);
% score_ARA = zeros(Ny,Nx);
% 
...
% 
%         % Spectre du pixel
%         S = squeeze(cube_sub(iy,ix,:));
% 
%         % Suppression baseline locale
%         S = S - min(S);
% 
%         % Rejeter les pixels trop faibles
%         if max(S) < 0.02
%             continue;
%         end
% 
%         % Normalisation pour comparer uniquement la forme
%         S = S / norm(S);
% 
%         % Corrélation avec les modèles
%         c_ACC = dot(S, g_ACC);
%         c_ARA = dot(S, g_ARA);
% 
%         score_ACC(iy,ix) = c_ACC;
%         score_ARA(iy,ix) = c_ARA;
% 
%         % Critère renforcé pour l'ACC
%         % (car phase minoritaire)
%         if c_ACC > 0.92 && c_ACC > c_ARA + 0.03
%             phase_map(iy,ix) = 1;   % ACC
% 
%         elseif c_ARA > 0.90
%             phase_map(iy,ix) = 2;   % Aragonite/Calcite
%         end
% 
%     end
% end
...
% axis image;
% colorbar;
% title('Carte de phase');
% colormap([0 0 0;      % bruit
%           1 0 0;      % ACC rouge
%           0 0 1]);    % Aragonite bleu
% 
% figure; hold on;
% 
% mask_ACC = score_ACC > score_ARA;
% mask_ARA = score_ARA > score_ACC;
% 
% % -------- Spectres Aragonite --------
% [y_ARA,x_ARA] = find(mask_ARA);
% 
% for k = 1:length(x_ARA)
%     spectrum = squeeze(I_corr(y_ARA(k),x_ARA(k),:));
%     plot(wavenumber, spectrum, 'b');
% end
% 
% 
% % -------- Spectres ACC --------
% [y_ACC,x_ACC] = find(mask_ACC);
% 
% for k = 1:length(x_ACC)
%     spectrum = squeeze(imgs(y_ACC(k),x_ACC(k),:));
%     plot(wavenumber, spectrum, 'r');
% end
% 
% 
...
% 
% legend(['Aragonite (' num2str(length(x_ARA)) ' pixels)'], ...
%        ['ACC (' num2str(length(x_ACC)) ' pixels)'])
% 
% title('Pixel spectra classified by chemical phase')
% grid on

%% Choose ROI bckg, sig 1 and 2 AND plot spectru
...
x_sig2=9.201558e+01; y_sig2=5.270255e+01; width_sig2=3.130312e+00; height_sig2=3.314448e+00;
    ...
    fprintf('x_sig1=%d; y_sig1=%d; width_sig1=%d; height_sig1=%d;\n', ...
            pos_sig1(1), pos_sig1(2), ...
            pos_sig1(3), pos_sig1(4));

    pos_sig2 = roi_sig2.Position;
    fprintf('x_sig2=%d; y_sig2=%d; width_sig2=%d; height_sig2=%d;\n', ...
            pos_sig2(1), pos_sig2(2), ...
            pos_sig2(3), pos_sig2(4));
end
...
%% map carbonate phases 

tau_fwhm = 15;

[A, offset_fit, valid_pixel, fit_spectrum, ...
          residual, R2, RMS, SNR, phase_names] = ...
...
[phase_map, A_maps, ratio_maps] = ...
    ...
    n_ch, ...              % canal
    tau_fwhm, ...          % FWHM réponse instrumentale
    true, ...              % ACC
    false, ...             % CCHH
    false, ...             % MHC
    false, ...             % Vaterite
    false, ...             % Aragonite
    true, ...              % Calcite
    3, ...                 % seuil bruit = 3 sigma
    0.05, ...              % seuil ACC/Calcite
    true);                 % affichage
...
% --- Clean data ---
valid = isfinite(sp1_sorted) & isfinite(wavenumber_sorted(:));
...
I3bis = I3;
wavenumber_sorted_bis = wavenumber_sorted;
sp1_sorted_bis = sp1_sorted;
sp2_sorted_bis = sp2_sorted;
...
save(fname_mat, 'I3bis', 'sp1_sorted_bis','sp2_sorted_bis', 'wavenumber', 'n_ch', '-v7.3');
