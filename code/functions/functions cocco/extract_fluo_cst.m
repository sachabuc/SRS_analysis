function [A,tau,Imean,C] = extract_fluo_cst(imgs, nfluo,xb,yb,xf,yf,wf,hf)
    ...
    if xf==0 
        ...
        fprintf('Coordonnées du ROI FLUO : [x=%.2f, y=%.2f, largeur=%.2f, hauteur=%.2f]\n', ...
        roi_sig_coords(1), roi_sig_coords(2), roi_sig_coords(3), roi_sig_coords(4));    
    else 
        roi_sig = drawrectangle('Position', [xf, yf, wf, hf], 'Color', 'r');
        mask_sig = createMask(roi_sig);
    end
    ...
    plot(t_fit, fitresult.A * exp(-t_fit / tau) + C , 'r-', 'LineWidth', 2);

    legend('Data', sprintf('Fit exp (A = %.2f, \\tau = %.2f, C = %.2f)', A, tau,C), 'Location', 'best');

    % Affichage console
    % disp('paramètres fit fluo:')
    % fprintf('Tau extrait = %.4f\n', tau);
    % fprintf('A extrait = %.4f\n', A);
    % fprintf('C extrait = %.4f\n', C);
    % fprintf('R^2 = %.4f\n', gof.rsquare);
