function [phase_map, composition_map] = segment_carbonate_phases_bis( ...
...
%     Comparaison des DEUX plus grandes amplitudes BRUTES parmi les
%     phases actives (A_sorted(1) >= A_sorted(2) apres tri) :
...
%                       sur les phases actives a amplitude > 0, NaN si exclu)

if nargin < 9 || isempty(display_figures), display_figures = true; end
...
for iy = 1:n_y
    for ix = 1:n_x
        ...
        if numel(A_sorted) > 1 && ~(A_sorted(1) > alpha_dominance*A_sorted(2))
            ...
            label_map(iy,ix) = active_idx(order(1));
        end
