...
main_dir = 'C:\Users\sacha.bucourt\Documents\Data lab\spectres\campagne cocco\260113\cell5'
sub_dir  = '';
...
%Contrast
    ...
    I2 = squeeze(I_corr(n2,:,:)); % poly 1 sup 
    I3 = squeeze(I_corr(n3,:,:)); % poly 1 
    % I4 = squeeze(I_corr(n4,:,:)); % poly 2
    % I5 = squeeze(I_corr(n5,:,:)); % poly 2 sup 
    % C=(I1-I2)./(I1+I2);
    C1=(I1-I2);
    C3=(I3-I2);
    % C4=(I4-I2);
    ...
    % 
    % subplot(3,3,8);
    % imagesc(I4-I5); caxis([-1 2]); colorbar;
    % title('I4-I5');
    % 
    % subplot(3,3,9);
    % imagesc(I2-I3); caxis([-1 2]); colorbar;
    % title('I2-I3');
...
for iz = 1:length(z_list)
    ...
    % Tracé avec un marqueur distinct et une couleur en dégradé de gris
    marker_idx = mod(iz-1, length(markers)) + 1;
    plot(wavenumber_sorted, sp_sorted, ...
         'LineWidth', 1.2, ...
         'Marker', markers{marker_idx}, ...
         'MarkerSize', 6, ...
         'MarkerIndices', 1:20:length(wavenumber_sorted), ...
         'Color', [gray_shades(iz) gray_shades(iz) gray_shades(iz)]);
    % [gray_shades(iz) gray_shades(iz) gray_shades(iz)] crée une couleur RGB en niveaux de gris
