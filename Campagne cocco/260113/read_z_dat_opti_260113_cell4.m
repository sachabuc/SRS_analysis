...
main_dir = 'C:\Users\sacha.bucourt\Documents\Data lab\spectres\campagne cocco\260113\cell4'
sub_dir  = '';
...
wavelengths = [927.1,920.9,920,923.6,961.5,952.1];
...
if contrast == true
    ...
    figure(100); imagesc(squeeze(sum(I_raw,1))); colorbar;
    x1_s = 120;
    x1_e = 140;
    x2_s = 120;
    x2_e = 140;
...
%Contrast
    ...
    I2 = squeeze(I_corr(n2,:,:)); % phosphate 1 
    I3 = squeeze(I_corr(n3,:,:)); % phosphate 1 inf
    I4 = squeeze(I_corr(n4,:,:)); % phosphate 2
    I5 = squeeze(I_corr(n5,:,:)); % phospjhate 2 sup
    % C=(I1-I2)./(I1+I2);
    % C1=(I1-I2)
    % C3=(I3-I2);
    % C4=(I4-I2);
    ...
    subplot(3,3,4);
    imagesc(I2); caxis([-1 1]); colorbar;
    title('phosphate 1 (I2)');

    subplot(3,3,5);
    imagesc(I3); caxis([-1 1]); colorbar;
    title('phosphate 1 inf (I3)');

    subplot(3,3,7);
    imagesc(I4); caxis([-1 1]); colorbar;
    title('phosphate 2 (I4)');

    subplot(3,3,8);
    imagesc(I5); caxis([-0.5 1]); colorbar;
    title('phosphate 2 sup (I5)');

    subplot(3,3,6);
    imagesc((I2-I3)./(I2+I3)); caxis([-1 1]); colorbar;
    title('(I2-I3)./(I2+I3)');

    subplot(3,3,9);
    imagesc((I4-I5)./(I4+I5)); caxis([-1 1]); colorbar;
    title('(I4-I5)./(I4+I5)');

    subplot(3,3,3);
    imagesc((I2-I1)./(I1+I2)); caxis([-0.5 1]); colorbar;
    title('(I2-I1)./(I2+I1)');

    subplot(3,3,2);
    imagesc((I4-I1)./(I1+I4)); caxis([-0.5 1]); colorbar;
    title('(I4-I1)./(I4+I1)');
...
fig_dir = fullfile(sub_dir_save, 'figures');
...
for k = 1:numel(fig_nums)
    ...
    if ishandle(fig_nums(k))
        hFig = figure(fig_nums(k));  % active la figure
        fname = fullfile(fig_dir, sprintf('figure_%d.png', fig_nums(k)));
        exportgraphics(hFig, fname, 'Resolution', 300);
        fprintf('Saved figure %d → %s\n', fig_nums(k), fname);
    else
        warning('Figure %d does not exist, skipped.', fig_nums(k));
    end
