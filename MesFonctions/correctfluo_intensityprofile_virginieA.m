function carto_nofluoA = correctfluo_intensityprofile_virginieA(imgs,nfluo,A,tau,C)
    ...
    alpha_cal = (A*exp(-time(1) / tau) + C )./( A*exp(-time(nfluo) / tau) + C)
    alpha_fluo = (A*exp(-time(2) / tau) + C )./( A*exp(-time(nfluo) / tau) + C)
    alpha_acc = (A*exp(-time(3) / tau) + C )./( A*exp(-time(nfluo) / tau) + C)
    alpha_acchira = (A*exp(-time(4) / tau) + C )./( A*exp(-time(nfluo) / tau) + C)
    % alpha_fluo2 = (A*exp(-time(5) / tau) + C )./( A*exp(-time(nfluo) / tau) + C)
